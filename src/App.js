...
const App = () => {
  ...
  //     //   ]);
  //     // }, []);
  //     // const questionsMarkup = questions.map((ques) => (
  //     //   <Questions question={ques} key={ques + Date.now()} />
  //     // setQuestions(navigator.userAgent);
  ...
  //       var loadingTask = pdfjsLib.getDocument(reader.result);
  //       loadingTask.promise.then(function (pdf) {
  //         console.log("PDF loaded", pdf.numPages);
  //       });
  //     };
      ...
      {!viewer && (
        <>
          <input
            ...
            value={url}
          />
          <button onClick={() => setViewer(true)}>Open file</button>
        </>
      )}
